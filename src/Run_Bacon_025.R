...
Bacon_parallel <- function(...) {
  ...
  age.mods.interp <- as.ffdf(hamstr:::predict.hamstr_bacon_fit(run, depth = seq(0,clength$corelength, by = 0.25)))
  rm(run)
  gc()
  while (max(age.mods.interp$iter) < 5001) {
    diff_iter <- 5001 - (max(age.mods.interp$iter) - min(age.mods.interp$iter))
    ...
    age.mods.interp <- as.ffdf(hamstr:::predict.hamstr_bacon_fit(run, depth = seq(min(run$pars$d.min),clength$corelength, by = 0.25)))
    rm(run)
    gc()
  } 
  gc()
  age.mods.interp <- subset(age.mods.interp, iter < 5001)
...
if (no_cores < length(CoreIDs)) {
  cl <- makeCluster(no_cores*0.8, outfile = "")} else {cl <- makeCluster(length(CoreIDs), outfile = "")} 
registerDoSNOW(cl)
seed <- 210329
...
Bacon_core_results <- foreach(i = seq_id_all 
  ...
  tryCatch(
    Bacon_parallel(i, Bacon_Frame, CoreIDs, CoreLengths)
    ,error = function(e){
