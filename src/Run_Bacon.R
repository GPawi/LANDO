...
if (length(CoreIDs) == 1) {
  ...
  age.mods.interp <- predict(run, depth = seq(0,clength$corelength, by = 1)) # for meter: by = 100
  rm(run)
  gc()
  ...
  #Bacon_core_results <- pivot_wider(as.data.frame(age.mods.interp), names_from = iter, values_from = age)
  ...
  Bacon_core_results <- result_individual_core
